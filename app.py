...
#----------- File is uploaded -------------
upload_file = st.sidebar.file_uploader("Upload your WhatsApp Chat file (.txt):")
if upload_file is not None:
    ...
    if menu == 'Most Active Users':
        if user == 'Overall':
            ...
            pu = pd.DataFrame(pu)
            pu.rename(columns={'index':'Name', 'User':'%_of_msg'}, inplace=True)
            
            # remove group_notification from the dataframe
            value = value[value.index != 'group_notification']
            ...
            with col1:
                st.dataframe(value)
            with col2:
                fig = px.pie(pu, values='%_of_msg', names='Name', width=400, height=400)
                st.plotly_chart(fig)
    ...
    st.markdown('* Sourav Chakraborty - 20BCB0054')
